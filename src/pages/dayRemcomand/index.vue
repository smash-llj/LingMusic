    ...
    <div class="listContainer">
        ...
        <div class="songContainer">
            ...
            <div class="songItem" v-for="(item, index) in dayResult" :key="index">
                <div @click="player(item, index)" class="links" :class="item.isplay ? 'play' : ''">
                    <img v-lazy="item.al.picUrl">
                    <p>{{ item.name }}</p>
                    <p>{{ item.ar[0].name }}</p>
                    <p>{{ item.al.name }}</p>
                </div>
                <p @click="item.isLike = !item.isLike"
                    :class="!item.isLike ? 'iconfont icon-xihuan' : 'iconfont icon-xihuan1'"></p>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getDayRecommand() {
            let res = await needLogin('/recommend/songs')
            let arr = res.data.dailySongs
            arr.forEach(item => {
                item.isLike = false
                item.isplay = false
            })
